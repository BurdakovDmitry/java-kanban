...
class PrioritizedHandlerTest {
    ...
    public void testGetPrioritized() throws IOException, InterruptedException {
        ...
        assertEquals(task1, listTask.getFirst(), "Некорректное имя задачи");
        assertEquals(task3, listTask.get(1), "Некорректное имя задачи");
        assertEquals(task2, listTask.getLast(), "Некорректное имя задачи");
    }

    @Test
    public void testGetPrioritizedEmpty() throws IOException, InterruptedException {
        final Task task4 = new Task("Task4", StatusTask.NEW, "Description1",
    ...
    @Test
    public void testGetPrioritizedErrorURI() throws IOException, InterruptedException {
        manager.createTask(task1);
        manager.createTask(task2);
        manager.createTask(task3);

        URI url = URI.create("http://localhost:8080/prioritized/error");
        HttpRequest request = HttpRequest.newBuilder()
                .uri(url)
                .GET()
                .build();
